    ...
    Effect.gen(function* () {
      ...
      if (proc.stdin) {
        ...
        yield* Effect.tryPromise({
          try: () => Promise.resolve(stdin.write(directoryList)),
            ...
            new UnknownError({ message: `Failed to write to fzf stdin: ${error}`, details: `Failed to write to fzf stdin: ${error}` }),
        });

        yield* Effect.tryPromise({
          try: () => Promise.resolve(stdin.end()),
          catch: (error) =>
            new UnknownError({ message: `Failed to close fzf stdin: ${error}`, details: `Failed to close fzf stdin: ${error}` }),
        });
      }
      ...
      const exitCode = yield* Effect.tryPromise({
        try: () => proc.exited,
          ...
          new UnknownError({ message: `Failed to wait for fzf process: ${error}`, details: `Failed to wait for fzf process: ${error}` }),
      });
      ...
      if (exitCode === 0 && proc.stdout) {
        const output = yield* Effect.tryPromise({
          try: () => new Response(proc.stdout).text(),
            ...
            new UnknownError({ message: `Failed to read fzf output: ${error}`, details: `Failed to read fzf output: ${error}` }),
        });
        return output.trim();
      }
      ...
      Effect.mapError((error) =>
        error instanceof UnknownError
          ? error
          : new UnknownError({ message: `Fzf selection failed: ${error}`, details: `Fzf selection failed: ${error}` }),
      ),
    ),
});
