  ...
  return {
    ...
    performHealthCheck: (): Effect.Effect<HealthCheckResult, never> =>
      Effect.gen(function* () {
        ...
        const dockerServices = makeDockerServicesLive(shell, fs, hostPaths, enabledServices);
...
export const createDockerServicesLiveLayer = (enabledServices?: readonly ServiceName[]) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return makeDockerServicesLive(shell, fs, hostPaths, enabledServices);
    }),
  );
...
export const DockerServicesLiveLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fs = yield* FileSystemTag;
    const hostPaths = yield* HostPathsTag;
    return makeDockerServicesLive(shell, fs, hostPaths, getEnabledServices(config.services));
  }),
);
