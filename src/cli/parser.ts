...
export interface CliParser {
  parseAndExecute(args: string[], appLayer?: Layer.Layer<any, any, any>): Promise<void>;
}

// Pure functions for CLI parsing and execution
const buildCommandString = (commandSpec: CliCommandSpec): string => {
...
const buildAliasCommandString = (alias: string, commandSpec: CliCommandSpec): string => {
  let command = alias;
...
const buildCommandOptions = (yargs: Argv, commandSpec: CliCommandSpec): Argv => {
  let builder = yargs;
...
const registerCommand = (
  ...
  yargsInstance.command(
    ...
    (yargs) => buildCommandOptions(yargs, commandSpec),
    (argv) => executeCommand(commandSpec, argv, appLayer),
  );
  ...
  if (commandSpec.aliases) {
    for (const alias of commandSpec.aliases) {
      yargsInstance.command(
        buildAliasCommandString(alias, commandSpec),
...
// Factory function to create CLI parser
export const createCliParser = (commands: CliCommandSpec[], metadata?: CliMetadata): CliParser => {
  return {
    async parseAndExecute(args: string[], appLayer?: Layer.Layer<any, any, any>): Promise<void> {
      // Get the app layer if not provided
      const layer = appLayer || (await import("../wiring")).getDefaultAppLayer();
      ...
      for (const commandSpec of commands) {
        registerCommand(yargsInstance, commandSpec, layer);
      }

      // Parse arguments and execute
      await yargsInstance.parseAsync(args);
    },
  };
};
