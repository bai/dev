...
import { HealthCheckPortTag, type HealthCheckPort } from "../../domain/ports/health-check-port";
  ...
  const scheduleHealthChecks = (): Effect.Effect<void, HealthCheckError> =>
    Effect.gen(function* () {
      ...
      if (shouldRun) {
        ...
        yield* healthCheckPort.runHealthChecksBackground();
...
export const HealthCheckSchedulerLiveLayer = Layer.effect(
  HealthCheckSchedulerTag,
  Effect.gen(function* () {
    const healthCheckPort = yield* HealthCheckPortTag;
    const pathService = yield* PathServiceTag;
    return makeHealthCheckSchedulerLive(healthCheckPort, pathService);
  }),
);
